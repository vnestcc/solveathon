...
const events = [
  ...
  },
  {
    ...
    title: 'Event Delta',
    desc: 'Morbi lectus risus, iaculis vel, suscipit quis, luctus non, massa. Fusce ac turpis quis ligula lacinia aliquet.',
    img: 'https://images.unsplash.com/photo-1454023492550-5696f8ff10e1',
  },
];

// Responsive image component with lazy loading
const ResponsiveImage = ({ src, alt, className, loading = "lazy" }) => {
  const baseUrl = src;
  
  return (
    <picture className={`${className}-container`}>
      {/* WebP format with responsive sizes */}
      <source 
        type="image/webp"
        srcSet={`${baseUrl}?auto=format&fit=crop&fm=webp&w=300&q=80 300w, 
                 ${baseUrl}?auto=format&fit=crop&fm=webp&w=600&q=80 600w, 
                 ${baseUrl}?auto=format&fit=crop&fm=webp&w=900&q=80 900w, 
                 ${baseUrl}?auto=format&fit=crop&fm=webp&w=1200&q=80 1200w`}
        sizes="(max-width: 768px) 100vw, (max-width: 1024px) 60vw, 50vw"
      />
      
      {/* Fallback with responsive srcSet */}
      <img 
        src={`${baseUrl}?auto=format&fit=crop&w=600&q=80`}
        srcSet={`${baseUrl}?auto=format&fit=crop&w=300&q=80 300w, 
      ...
      <div className={`pre-events-content ${isMobile ? 'mobile' : 'desktop'}`}> 
        {isMobile ? (
              ...
              {events.map((event, i) => (
                i !== activeIdx && (
                  <ResponsiveImage
                    ...
                    src={event.img}
                    alt={event.title}
                    className="pre-events-image-preload"
                    loading="lazy"
                  />
                )
              ))}
          ...
          <div className="pre-events-row">
            ...
            {/* Preload other images for smooth transitions */}
            <div style={{ display: 'none' }}>
              {events.map((event, i) => (
                i !== activeIdx && (
                  <ResponsiveImage
